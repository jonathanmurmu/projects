#!/usr/bin/python
"""Sending a recovery email containing credentials to the given email id."""
import smtplib
import MySQLdb
import cgi
import cgitb
...
def recovery():
    ...
    sql = "SELECT IF( EXISTS(SELECT user_detail_id FROM user_detail \
        WHERE email='{0}') ,1,0)".format(to)
    cursor.execute(sql)
    ...
    if data == 1:
        ...
        try:
            ...
            try:
                ...
                print '<h3>Username and password is sent to your\
                 email address</h3>'
